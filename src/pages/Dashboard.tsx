...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [contactsResult, companiesResult, licensesResult, expiringResult] = await Promise.all([supabase.from('contacts').select('id', {
          count: 'exact',
          head: true
        }).is('deleted_at', null), supabase.from('companies').select('id', {
          count: 'exact',
          head: true
        }).is('deleted_at', null), supabase.from('licenses').select('id', {
          count: 'exact',
          head: true
        }).is('deleted_at', null).eq('status', 'active'), supabase.from('licenses').select('id', {
          count: 'exact',
          head: true
        }).is('deleted_at', null).eq('status', 'active').gte('end_date', new Date().toISOString().split('T')[0]).lte('end_date', new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])]);
        setStats({
  ...
  if (loading) {
    return <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => <Card key={i} className="card-elevated animate-pulse">
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {statCards.map(card => <Card key={card.title} className="card-elevated hover:shadow-lg transition-shadow">
