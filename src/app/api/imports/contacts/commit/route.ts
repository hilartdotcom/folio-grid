...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!authHeader) {
      throw new ImportError('UNAUTHORIZED', 'Authentication required', 401);
    }
    ...
    console.log('CONTACTS_COMMIT_START', { 
      correlationId, 
      fileName, 
    ...
    const { data: attempt, error: attemptError } = await supabase
      ...
      .insert({
        ...
        source_url: !contentType.includes('multipart/form-data') ? req.url : null,
        status: 'running',
        total_rows: 0,
        valid_rows: 0,
        error_count: 0,
    ...
    console.error('CONTACTS_COMMIT_ERROR', { 
      correlationId, 
      code, 
      message, 
      stack: e?.stack 
